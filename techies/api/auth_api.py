...
from techies.api.user import get_user
from frappe.utils import cint

import frappe
# from nfmagnetize.api.cart import get_party

logger=frappe.logger("a_jwt", file_count=1)

# @frappe.whitelist()
# def get_logged_user():
# 	current_user=frappe.session.user
# 	# current_user="mohammed@mugen.ae"
# 	if current_user:
# 		if frappe.db.exists("Employee", {"user_id": current_user}):
# 			employee = frappe.get_cached_doc("Employee", {"user_id": current_user})
# 		else:
# 			employee = None
# 		userimage=frappe.get_cached_value("User", current_user, "user_image")
# 		roles = frappe.get_roles(current_user)
# 		return {"current_user": current_user,"user_image":userimage, "employee": employee, "roles": roles}
