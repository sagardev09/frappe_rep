...
from frappe.utils import cint, get_url, get_datetime, now_datetime
...
# implementing functionality for password reset
@frappe.whitelist(allow_guest=True)
def reset_password(user, serverUrl, old_password=None):
  ...
  encoded_server_url = quote(serverUrl, safe='')
  link = f"https://apps.mugen.ae/en/forgot-password?key={key}&serverUrl={encoded_server_url}&user={user.name}" 
